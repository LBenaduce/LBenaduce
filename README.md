# LBenaduce
# Luiz Benaduce

**Front-End Developer (TripleTen)** • JavaScript • HTML • CSS • REST APIs

Front-End Developer focused on building responsive, accessible, and maintainable web interfaces.  
I enjoy creating clean UI experiences, writing organized code, and integrating real-world APIs.

---

## Summary

- Strong foundation in **HTML5 / CSS3 (BEM)** and **JavaScript (ES6+)**
- Experience building projects using **modular architecture** and **JavaScript classes**
- Integration with **REST APIs** using **fetch** and **async/await**
- Comfortable with **Git & GitHub** workflows
- Focus on clean code, responsive design, and UI/UX fundamentals

---

## Skills

**Languages & Core**
- JavaScript (ES6+)
- HTML5
- CSS3

**Tools**
- Git / GitHub
- VS Code

**Concepts**
- Responsive Web Design
- DOM Manipulation
- OOP (JavaScript Classes)
- Form Validation
- REST API Integration
- Code Organization (modules / reusable components)

---

## Featured Project

### Around US — Web App (TripleTen Project)
A responsive social-style web application where users can edit profile data, update avatar, create cards, and like/unlike posts.  
Built with a modular structure using JavaScript classes and integrated with a REST API.

**Key Features**
- Profile editing + avatar update
- Add/remove cards with confirmation popup
- Like/unlike system
- Client-side form validation
- API requests (GET, POST, PATCH, DELETE)

**Links**
- Repository: https://github.com/LBenaduce/web_project_around  
- Live Demo: https://lbenaduce.github.io/web_project_around/

---

## Contact

- LinkedIn: https://www.linkedin.com/in/luiz-benaduce-014740275/
- GitHub: https://github.com/LBenaduce

## 🤖 Languages & Technologies

<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="44" alt="HTML5" />
  <img width="14" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="44" alt="CSS3" />
  <img width="14" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="44" alt="JavaScript" />
  <img width="14" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" height="44" alt="Git" />
  <img width="14" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" height="44" alt="GitHub" />
  <img width="14" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" height="44" alt="VS Code" />
  <img width="14" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="44" alt="Node.js" />
</div>
